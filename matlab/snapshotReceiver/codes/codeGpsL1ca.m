% c = codeGpsL1ca([ prn ])
%
% Generates the GPS C/A spreading code of a given satellite (PRN).
%
% Parameters:
% prn........ (optional) list of the PRN's to generate (default: 1:32)
%
% Returns:
% c.......... the spreading codes (N x 1023)
%
function c = codeGpsL1ca(prn)

if ~exist('prn', 'var')
    prn = 1:32;
end

c = NaN(length(prn), 1023);
for n=1:length(prn)
    c(n,:) = generateCAcode(prn(n));
end
